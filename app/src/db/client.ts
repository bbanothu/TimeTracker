...
export function getTimeEntryByIdForHistory(id: string): TimeEntry | null {
...
function loadTagsForEntries(entryIds: string[]): Map<string, Tag[]> {
  ...
  const rows = getDb().getAllSync<{
    ...
    `SELECT tet.entry_id, t.id, t.name, t.color, t.parent_id, t.include_in_analytics, t.description
     FROM tags t
     INNER JOIN time_entry_tags tet ON tet.tag_id = t.id
     WHERE tet.user_id = ? AND tet.entry_id IN (${placeholders})`,
    [userId, ...entryIds],
  );
  ...
  for (const row of rows) {
    ...
    list.push(
      rowToTag({
        ...
        include_in_analytics: row.include_in_analytics,
...
function mapEntryRows(rows: EntryRow[]): TimeEntry[] {
  const tagsByEntry = loadTagsForEntries(rows.map((row) => row.id));
  return rows.map((row) => ({
    id: row.id,
    startedAt: row.started_at,
    endedAt: row.ended_at,
...
export function getHistoryEntriesPage(
  filters: {
...
): { entries: TimeEntry[]; totalCount: number; page: number; totalPages: number } {
  const userId = requireUserId();
  const database = getDb();

  const where: string[] = ['te.user_id = ?', 'te.ended_at IS NOT NULL'];
  const params: (string | number)[] = [userId];

  if (dateRange) {
    where.push('te.ended_at >= ? AND te.started_at <= ?');
  ...
  if (filters.tagId) {
    where.push(
      `EXISTS (
        ...
        WHERE tet.entry_id = te.id AND tet.user_id = te.user_id AND tet.tag_id = ?
      )`,
    );
  ...
  const countRow = database.getFirstSync<{ count: number }>(
    `SELECT COUNT(*) as count FROM time_entries te WHERE ${whereSql}`,
    params,
  );
  ...
  const rows = database.getAllSync<EntryRow>(
    `SELECT te.id, te.started_at, te.ended_at, te.source, te.geofence_id,
            te.stop_latitude, te.stop_longitude, te.details
